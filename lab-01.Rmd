...
```{r Q3}
visitors |> arrange(desc(n_2022)) |> view()
...
```{r Q5}
visitors |> 
  filter(setting == "O", region == "Yorkshire and the Humber", admission == "Members", n_2022 > 100000) |> view()
...
visitors %>%
 group_by(region) %>%
 summarise(num_attractions = n()) |> 
 filter(num_attractions > 50)
...
<!-- NA stands for not applicable. In the original data_set "visitors" we have NA values in the n_2022 column so in our summarize function we are asking the RStudio to take the mean of values with NA which is not possible so the whole function returns NA -->
...
Most likely the data is positively skewed. There are a few values that are much larger than other which causes the mean to rise a lot while the median will stay in the middle.
...
Our inter-quartile estimates arent very good when we visulize the actual numbers.
...
visitors_with_nations |> 
  ...
  summarise(
    ...
    proportion = (free_attractions / total_attractions)*100
    ) |> 
  view()
...
visitors_with_nations |> 
  ...
  summarise(
    ...
    n_2022 = sum(n_2022, na.rm = TRUE),
    n_change = ((n_2022 - n_2021)/n_2021)*100
  ) |> 
  arrange(n_change)
